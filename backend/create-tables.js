...
const client = new Client({
  ...
  port: 5432,
  user: 'postgres',
  password: 'reshana2003',
  database: 'oncostage_db'
});
  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS "predictions" (
        ...
        CONSTRAINT "predictions_patientId_fkey" FOREIGN KEY ("patientId") REFERENCES "users"("id"),
        CONSTRAINT "predictions_doctorId_fkey" FOREIGN KEY ("doctorId") REFERENCES "users"("id")
      )
    `);
    ...
    const checkRes = await client.query(`
      ...
      WHERE table_name='predictions' LIMIT 5
    `);
    
    if (checkRes.rows.length > 0) {
      console.log('✅ Table verified with columns:', checkRes.rows.map(r => r.column_name).join(', '));
    }
